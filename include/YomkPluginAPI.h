 ...
 */
#include "YomkPluginInterface.h" /* ABI 契约：插件实例接口 */
#include "YomkPluginMeta.h"      /* ABI 契约：插件元数据结构 */
#include <YomkServer/YomkAPI.h> /* YOMK_INIT / YOMK_NEW_SERVICE / YOMK_REQUEST / YomkMkPtr */
...
#include "YomkPluginLoader.h"  /* 机制层服务声明（宿主注册用） */
#include "YomkPluginManager.h" /* 数据层服务声明 */
#include "YomkPluginMsgs.h" /* 消息数据类：结构体 + YomkMsg 注册 */
#include "YomkPluginSystemBuilder.h" /* 编排层服务声明 */

/* ------------------------- 用户 API 宏（风格对齐框架 YOMK_CONTEXT_INFO_*）
 * ------------------------- */

/* 一键注册三个扩展服务（宿主在 YOMK_INIT 后调用一次） */
#define YOMK_PLUGIN_NEW_SERVICES()                                             \
  do {                                                                         \
...
#define YOMK_PLUGIN_LOADER_LOAD_LIB(path)                                      \
  YOMK_REQUEST("/YomkPluginLoader/loadLib",                                    \
               YomkMkPtr(PluginPath, PluginPath{path}))
/* 卸载指定插件库（引用计数归零才真正 dlclose），返回 String(ok/msg) */
#define YOMK_PLUGIN_LOADER_UNLOAD_LIB(libId)                                   \
  YOMK_REQUEST("/YomkPluginLoader/unloadLib", YomkMkPtr(String, libId))
/* 查询单库元信息，返回 String(PluginMeta 序列化) */
#define YOMK_PLUGIN_LOADER_META(libId)                                         \
  YOMK_REQUEST("/YomkPluginLoader/meta", YomkMkPtr(String, libId))
/* 创建插件实例并注入 instance_name，返回
 * PluginInstance(进程内直接持有的接口指针) */
#define YOMK_PLUGIN_LOADER_CREATE(libId, instanceName, instanceFile)           \
  YOMK_REQUEST(                                                                \
      "/YomkPluginLoader/create",                                              \
...
#define YOMK_PLUGIN_LOADER_DELETE(instance)                                    \
  YOMK_REQUEST("/YomkPluginLoader/delete", YomkMkPtr(PluginInstance, instance))
/* 查询 Loader 服务版本，返回 String */
#define YOMK_PLUGIN_LOADER_VERSION()                                           \
  YOMK_REQUEST("/YomkPluginLoader/version", nullptr)

/* Loader：机制层内省（已加载库列表 / 单库元信息 / 全量 dump） */
#define YOMK_PLUGIN_LOADER_INFO_LIBS()                                         \
  YOMK_REQUEST("/YomkPluginLoader/libs", nullptr)
#define YOMK_PLUGIN_LOADER_INFO_LIB(libId)                                     \
  YOMK_REQUEST("/YomkPluginLoader/lib", YomkMkPtr(String, libId))
#define YOMK_PLUGIN_LOADER_INFO_ALL()                                          \
  YOMK_REQUEST("/YomkPluginLoader/all", nullptr)

/* ------------------------- Manager：数据层 ------------------------- */

/* 加载 so 并登记插件表（不自动建实例），返回 String(libId) */
#define YOMK_PLUGIN_MANAGER_LOAD(path)                                         \
  YOMK_REQUEST("/YomkPluginManager/load",                                      \
               YomkMkPtr(PluginPath, PluginPath{path}))
/* 尝试卸载：有存活实例则失败并返回实例数，返回 String(ok/msg) */
#define YOMK_PLUGIN_MANAGER_TRY_UNLOAD(libId)                                  \
  YOMK_REQUEST("/YomkPluginManager/try_unload", YomkMkPtr(String, libId))
/* 强制卸载：先销毁全部实例再卸载，返回 String(ok/msg) */
#define YOMK_PLUGIN_MANAGER_FORCE_UNLOAD(libId)                                \
  YOMK_REQUEST("/YomkPluginManager/force_unload", YomkMkPtr(String, libId))
/* 经 Loader 创建实例并登记实例明细，返回 InstanceInfo */
#define YOMK_PLUGIN_MANAGER_CREATE_INSTANCE(libId, instanceName, instanceFile) \
  YOMK_REQUEST(                                                                \
      "/YomkPluginManager/create_instance",                                    \
      YomkMkPtr(CreateReq, CreateReq{libId, instanceName, instanceFile}))
/* 销毁指定实例并注销明细，返回 String(ok/msg) */
#define YOMK_PLUGIN_MANAGER_DESTROY_INSTANCE(libId, instanceName)              \
  YOMK_REQUEST("/YomkPluginManager/destroy_instance",                          \
               YomkMkPtr(DestroyReq, DestroyReq{libId, instanceName}))
/* 查询插件列表（libId 为空查全部），返回 StringArray */
#define YOMK_PLUGIN_MANAGER_LIST()                                             \
  YOMK_REQUEST("/YomkPluginManager/list", nullptr)
#define YOMK_PLUGIN_MANAGER_LIST_LIB(libId)                                    \
  YOMK_REQUEST("/YomkPluginManager/list", YomkMkPtr(String, libId))
/* 查询实例明细（libId 为空查全部），返回 InstanceInfoArray */
#define YOMK_PLUGIN_MANAGER_LIST_INSTANCES()                                   \
  YOMK_REQUEST("/YomkPluginManager/list_instances", nullptr)
#define YOMK_PLUGIN_MANAGER_LIST_INSTANCES_LIB(libId)                          \
  YOMK_REQUEST("/YomkPluginManager/list_instances", YomkMkPtr(String, libId))
/* 查询 Manager 服务版本，返回 String */
#define YOMK_PLUGIN_MANAGER_VERSION()                                          \
  YOMK_REQUEST("/YomkPluginManager/version", nullptr)

/* Manager：数据层内省（插件列表 / 单插件元信息 / 全量 dump 含实例明细） */
#define YOMK_PLUGIN_MANAGER_INFO_PLUGINS()                                     \
  YOMK_REQUEST("/YomkPluginManager/plugins", nullptr)
#define YOMK_PLUGIN_MANAGER_INFO_PLUGIN(libId)                                 \
  YOMK_REQUEST("/YomkPluginManager/plugin", YomkMkPtr(String, libId))
#define YOMK_PLUGIN_MANAGER_INFO_ALL()                                         \
  YOMK_REQUEST("/YomkPluginManager/all", nullptr)

/* ------------------------- Builder：编排层 ------------------------- */

/* 按清单构建插件工程（yomk 清单驱动 cmake），返回 StringArray(构建产物 so 路径)
 */
#define YOMK_PLUGIN_BUILDER_BUILD(workflowPath)                                \
  YOMK_REQUEST("/YomkPluginSystemBuilder/build",                               \
               YomkMkPtr(BuildReq, BuildReq{workflowPath}))
/* 查询 Builder 服务版本，返回 String */
#define YOMK_PLUGIN_BUILDER_VERSION()                                          \
  YOMK_REQUEST("/YomkPluginSystemBuilder/version", nullptr)

/* Builder：编排层内省（全量 dump） */
#define YOMK_PLUGIN_BUILDER_INFO_ALL()                                         \
