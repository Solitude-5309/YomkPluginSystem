#!/bin/bash
# YomkPluginSystem 静态代码检查运行器（闭环10；日志+控制台双输出对齐 YomkRpc 同名脚本）
# 用法:
...
# 行为:
#   1. cppcheck 档: 只检测交付物 src/ include/ examples/，warning/performance/portability
#      零告警验收（--error-exitcode=1），先打印检测文件清单，扫描时过滤显示逐文件 Checking
#      进度行与告警行（完整 verbose 输出落盘日志）
...
if [ ${FAILED} -ne 0 ]; then
    ...
    echo "==========================================="
    echo "YomkPluginSystem 静态代码检查未通过" >> "${SUMMARY}"
    exit 1
fi
echo "==========================================="
echo " 静态代码检查全部通过（日志目录: ${LOG_ROOT}）"
echo "==========================================="
echo "YomkPluginSystem 静态代码检查全部通过" >> "${SUMMARY}"
exit 0
